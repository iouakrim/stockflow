...
const CATEGORIES = [
    ...
    "Produits Vétérinaires", "Emballages & Accessoires", "Autre",
]
...
function validateRow(row: DraftRow) {
    const errors: Record<string, string> = {}
    if (!row.name.trim() || row.name.trim().length < 2) errors.name = "Nom requis (min 2 car.)"
    if (row.cost_price !== "" && isNaN(parseFloat(row.cost_price))) errors.cost_price = "Nombre invalide"
    if (parseFloat(row.cost_price) < 0) errors.cost_price = "Doit être ≥ 0"
    if (row.selling_price !== "" && isNaN(parseFloat(row.selling_price))) errors.selling_price = "Nombre invalide"
    if (parseFloat(row.selling_price) < 0) errors.selling_price = "Doit être ≥ 0"
    if (!row.unit) errors.unit = "Requis"
    return errors
}
...
export function BulkEntryClient({
    ...
    const validRows = nonEmptyRows.filter(r => Object.keys(validateRow(r)).length === 0)
    ...
    const handleImport = async () => {
        ...
        if (successCount > 0) {
            toast.success(`${successCount} produit(s) importés avec succès`)
            try { localStorage.removeItem(DRAFT_KEY) } catch { /* ignore */ }
            if (errors.length === 0) {
        ...
        <div className="space-y-4">
            ...
            {draftRestored && !result && (
                <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-xs font-bold text-amber-600">
                    ...
                    Brouillon restauré depuis votre dernière session.
            ...
            {result && (
                <div className={`flex items-start gap-3 px-5 py-4 rounded-2xl border text-sm font-bold ${result.errors.length === 0
                    ...
                    <div>
                        <p>✓ {result.success} produit(s) importés</p>
                        {result.errors.length > 0 && (
            ...
            <div className="flex items-center gap-3 flex-wrap">
                <div className="flex items-center gap-2">
                    ...
                    >
                        <Plus className="h-3.5 w-3.5" /> +5 lignes
                    </Button>
                    <Button
                        variant="outline"
                        size="sm"
                        ...
                        className="h-9 rounded-xl border-primary/10 bg-card/40 font-black text-xs gap-1.5 hover:bg-primary/10 hover:text-primary"
                    >
                        <Plus className="h-3.5 w-3.5" /> +20 lignes
                    </Button>
                    <Button
                        variant="ghost"
                        size="sm"
                ...
                <div className="ml-auto flex items-center gap-3">
                    ...
                    >
                        {isImporting
                            ...
                            : <><Zap className="h-3.5 w-3.5" /> Importer {validRows.length > 0 ? validRows.length : ""} produit(s)</>
                        }
                    </Button>
                </div>
            </div>
            ...
            <div className="rounded-[2rem] border border-primary/10 bg-card/30 backdrop-blur-xl overflow-hidden shadow-2xl shadow-black/10">
                <div className="overflow-x-auto">
                    <table ref={tableRef} className="w-full text-sm border-collapse">
                        <thead className="bg-primary/[0.03] border-b border-primary/5">
                            <tr>
                                <th className="py-4 pl-6 pr-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left w-8">#</th>
                                ...
                                <th className="py-4 px-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left min-w-[180px]">Catégorie</th>
                                <th className="py-4 px-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left w-28">Unité</th>
                                <th className="py-4 px-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left w-32">
                                    Prix achat <span className="text-red-400">*</span>
                                </th>
                                <th className="py-4 px-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left w-32">
                                    Prix vente <span className="text-red-400">*</span>
                                </th>
                                {suppliers.length > 0 && (
                                    <th className="py-4 px-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 text-left min-w-[160px]">Fournisseur</th>
                                )}
                ...
                <div className="px-6 py-3 border-t border-primary/5 bg-primary/[0.02] flex items-center gap-3">
                    <p className="text-[10px] text-muted-foreground/40 font-bold uppercase tracking-widest">
                        Tab — cellule suivante &nbsp;·&nbsp; Entrée — nouvelle ligne
                    </p>
